...
func MakeServer(p Processor, s Store, a Addr) *Server {
	return &Server{p, s, string(a)}
}
...
func (s *Server) HandleUpload(w http.ResponseWriter, req *http.Request) {
	...
	bundleData := req.Body
	bundleData, err := s.processor.Process(bundleData)
	...
	if err != nil {
		http.Error(w, fmt.Sprintf("Error processing bundle: %s", err), http.StatusBadRequest)
		return
	}
